...
template <typename VertexType = FDTreeVertex>
class FDTree {
private:
    std::shared_ptr<VertexType> root_;

public:
    explicit FDTree(size_t num_attributes) : root_(std::make_shared<VertexType>(num_attributes)) {
    ...
    std::shared_ptr<VertexType> GetRootPtr() noexcept {
        return root_;
    }

    [[nodiscard]] VertexType const& GetRoot() const noexcept {
        return *root_;
    }

    std::shared_ptr<VertexType> AddFD(boost::dynamic_bitset<> const& lhs, size_t rhs) {
        VertexType* cur_node = root_.get();
    ...
    bool ContainsFD(boost::dynamic_bitset<> const& lhs, size_t rhs) {
        VertexType const* cur_node = root_.get();
    ...
    bool ContainsFdOrSpecial(boost::dynamic_bitset<> const& lhs, size_t rhs) const {
