...
std::unique_ptr<DynamicPositionListIndex> DynamicPositionListIndex::CreateFor(
    ...
    for (auto value_id : data) {
        ...
        if (!inverted_index.contains(value_id)) {
            clusters.emplace_back();
            inverted_index[value_id] = &clusters.back();
        }
        inverted_index[value_id]->insert(static_cast<int>(next_record_id++));
    }
...
void DynamicPositionListIndex::Insert(int const value_id) {
    int record_id = next_record_id_++;
